...
class SimplePathGen:
    def __init__(self) -> None:
        ...
        self._path_pub = rospy.Publisher('docking_path/plan', Path, queue_size=1)
    ...
    def gen_path(self):
        ...
        if marker_point is not None:
            ...
            x_end, y_end = marker_point.transform.translation.x, marker_point.transform.translation.y
            A = np.array([[x_start**2, x_start, 1],
                        [x_end**2, x_end, 1],
                        [2 * x_start, 1, 0]])
            B = np.array([y_start, y_end, 0])
            a, b, c = np.linalg.solve(A, B)
            x = np.arange(x_end, x_start, -interval)
            y = a * x**2 + b * x + c
            return x, y
        else:
...
if __name__ == '__main__':
    rospy.init_node('simple_path_generetor')
